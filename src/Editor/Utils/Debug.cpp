...
namespace Editor::Utils
{
	...
	void Debug::drawCollider(ColliderShape* collider, const Transform& transform, Shader* shader,
			const glm::vec3& color) const
	{
		...
		shader->setMat4("model", colliderMatrix);
		mesh->draw(shader);
	...
	/*void Debug::setRect(float x, float y, float width, float height)
	{
		this->x = x;
		this->y = y;
		this->width = width;
